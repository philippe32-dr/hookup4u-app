  ...
  Widget build(BuildContext context) {
    ...
    FirebaseMessaging.instance.getToken().then((token) async {
      ...
      if (!isPuchased) {
        ...
        if (Platform.isIOS) {
      getDevicePushTokenVoIP().then((voipToken) async {
        try {
           if (userProvider.currentUser!.voipToken != voipToken) {
          await firebaseFireStoreInstance
              .collection('Users')
              .doc(userProvider.currentUser!.id)
              .update({'voipToken': voipToken});
          debugPrint('Device voipToken: $voipToken');
          }
        } catch (e) {
    ...
    return PopScope(
     ...
     onPopInvokedWithResult: (bool didPop, dynamic result)  async {
      ...
      child: Scaffold(
        body: userProvider.currentUser?.isBlocked ?? false
            ? const BlockByAdmin()
